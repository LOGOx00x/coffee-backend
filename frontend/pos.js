...
function openModal(index) {
  ...
  document.getElementById("modal-toppings").innerHTML = toppings
    .map(
      (t) => `
    ...
    .map((item, i) => {
      ...
      const topText =
        item.toppingNames.length > 0
          ? `<br><span class="text-xs text-blue-500">+ ${item.toppingNames.join(", ")}</span>`
          : "";
      return `
  ...
  try {
    ...
    if (data.status === "success") {
      ...
      const newName = prompt(
        "ไม่พบเบอร์นี้ กรุณาพิมพ์ชื่อลูกค้าเพื่อสมัครสมาชิกใหม่:",
      );
      if (newName) {
